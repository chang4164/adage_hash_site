...
//index.html로부터 sub.html페이지로 처음 진입했을 때
...
var $contCover = document.querySelector("#cont .wrap .cont"); //내부 이미지 바꿀곳
var $contGroup = ""; //.cont라는 공간에 넣을 가상의 빈 문자열 테이터(봉투1)

var hash_apply = function(){
  if($hash_txt){ //hash로 받아온 값이 존재한다면! == $hash_txt.length > 0
    $main.style.backgroundImage = `url(./img/${$hash_txt}_banner.jpg)`;
    $title.textContent = $hash_txt;

    console.log($contGroup);

    $contGroup = ""; //sub.html 내부에서 상단 메뉴 클릭시, 기존에 누적된 쌓인 데이터들을 모두 재 초기화.
...
//var 약점은 기존에 작성된 변수명이 있다고 하더라도 새롭게 동일한 명칭으로 변수명을 적게되면 갱신(데이터의 완전 교체) 된다.
//const로 선언된 변수명은 고정값만을 가져올 수 있음. (갱신 불가 - 견고함, 경직된 부분, 객체, 배열, 함수)